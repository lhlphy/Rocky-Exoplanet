...
def specular_reflection(RV, camera, normal, r, alpha_max = -1):
    ...
    Args:
        RV (array): 反射向量。
        camera (array): 相机/观测向量。
        normal (array): 反射点的法向量。
        r: 行星半径相关参数。若为-1则自动计算最大角度。
        alpha_max: 最大入射角阈值，默认为-1（自动计算）。

    Returns:
        float: 镜面反射强度I除以B(T,lam)的值。若角度超过阈值则返回0。
    """
...
def reflection_plot(Intensity, Theta, id=0, type='specular'):
    '''绘制行星的反射区域'''
    np.save(f'temp/R{id}/plots/plot_{type}_' + str(int(Theta * 180 / np.pi)) + '.npy', Intensity.T)
    ...
    os.makedirs(f'temp/R{id}/plots', exist_ok=True)
    name = f'temp/R{id}/plots/plot_{type}_'+str(int(Theta*180/np.pi))+'.png'
    plt.savefig(name)
    plt.close()
