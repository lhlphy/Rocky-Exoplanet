...
if 'pl_orbsmax' in df_c.columns and 'pl_rade' in df_c.columns and 'pl_name' in df_c.columns:  
    ...
    fl_df['RSM'] = (fl_df['pl_rade'] / fl_df['pl_orbsmax'])**2 * Co1 * 10**(-fl_df['sy_kmag']/5) /1.744492e-06 *100
    ...
    fl_df['Tot_CR'] =  (fl_df['pl_rade']/fl_df['st_rad'])**2 *(R_E/R_Sun)**2 * B_func(fl_df['pl_eqt'] *1.1, lam_spec) / B_func(fl_df['st_teff'], lam_spec) *1e6 + fl_df['Spl_CR']
    sorted_df = fl_df.sort_values(by='RSM', ascending=False) 
    
    # 提取前10个恒星的name参数  
    top_10_names = sorted_df[['pl_name','Specular', 'Specular_corr','RSM', 'pl_eqt', 'Spl_CR','Tot_CR', 'liq_area']].head(30)  
